...
# @st.cache_data
def generate_random_points_in_polygon(_polygon: Polygon, num_points: int, seed: int = 10) -> List[Point]:
    '''Docstring'''
    np.random.seed(seed)
    # Create a GeoDataFrame with a single row for the polygon
    poly_gdf = gpd.GeoDataFrame(geometry=[_polygon])
    # Create a square grid of points covering the bounding box of the polygon
    bbox = _polygon.bounds
    side = np.sqrt(bbox[2]-bbox[0]) * \
        np.sqrt(bbox[3]-bbox[1]) / np.sqrt(num_points)
    x_coords = np.arange(bbox[0], bbox[2], side)
    y_coords = np.arange(bbox[1], bbox[3], side)
    xx, yy = np.meshgrid(x_coords, y_coords)
    points_grid = gpd.GeoDataFrame(
        geometry=gpd.points_from_xy(xx.ravel(), yy.ravel()))
    # Spatial join the grid with the polygon to keep only the points within the polygon
    points_within_poly = gpd.sjoin(points_grid, poly_gdf, op='within')
    # Rejection sampling to ensure num_points in the final list
    random_points = []
    while len(random_points) < num_points:
        # Generate a batch of random points within the polygon
        batch_size = num_points - len(random_points)
        batch_points = gpd.GeoDataFrame(geometry=[Point(p) for p in zip(np.random.uniform(
            bbox[0], bbox[2], batch_size), np.random.uniform(bbox[1], bbox[3], batch_size))])
        batch_points_within_poly = gpd.sjoin(
            batch_points, poly_gdf, op='within')
        # Add the batch of points to the final list
        random_points += list(batch_points_within_poly.geometry)
    # Truncate the list to num_points
    random_points = random_points[:num_points]
